...
const EMPTY_PROBLEM: ProblemForm = {
  name: "",
  description: "",
...
export default function AssignmentForm({
  ...
  const [problems, setProblems] = useState<ProblemForm[]>(
    initial?.problems ?? [{ ...EMPTY_PROBLEM }]
  );
  ...
  function updateProblem(pi: number, field: "name" | "description", value: string) {
  ...
  async function handleSubmit() {
    ...
    try {
      await onSubmit({ name, description, problems });
    } catch (err) {
            ...
            >
              ...
              <div className="border-t border-zinc-100 dark:border-zinc-800 pt-3 space-y-2">
                {problem.criteria.map((criterion, ci) => (
                  <div key={criterion.id ?? `new-${ci}`} className="flex items-center gap-2 group min-w-0">
                    <input
                      ref={
                        pi === focusProblemIdx &&
